...
class ProductManager {
	...
	// ====================================================
	async getProductById(paramsObj) {
		...
		return {
			error: `El valor del parámetro 'pid' ${pid} no es válido.`
		};
	}
	...
	async updateProduct(productId, updatedProductData) {
		...
		try {
			...
			if (productIndex === -1) {
				...
				return {
					message: "El valor del parámetro 'pid' no es válido.",
					error: null
			...
			return {
				message: `Producto actualizado correctamente.ID: ${productId}`,
				product: products
			};
		...
		try {
			...
			if (!idExists) {
				...
				return {
					...
					message: `El ID no existe: ${productId}`,
					error: null
				};
