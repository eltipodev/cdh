...
// NOTE: Pasado
///  GET mostrar todos loas productos     ///
////////////////////////////////////////////
router.get("/", async (req, res) => {
	try {
		const allProducts = await productManager.getProducts(req.query);
		if (!allProducts.length) {
			res.render("home", { products: [], titleSection: "No hay Productos", style: "products" });
		}
		...
		// 	allProducts
		// });
		res.render("home", { products: allProducts, titleSection: "Productos", style: "products" });
	}
	catch (error) {
		res.status(500).json({
			message: "😪 Ocurrió un error interno en el servidor.",
...
router.get("/:pid", async (req, res) => {
	...
	try {
		...
		const arrayDeProducts = [productById];

		!productById
			? res.render("home", { products: arrayDeProducts, titleSection: "Producto no se encontro" })
			: res.render("home", { products: arrayDeProducts, titleSection: "Producto Encontrado" });
